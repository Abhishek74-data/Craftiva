      ...
      <section className="relative overflow-hidden bg-walnut-dark">
        <div className="absolute inset-0">
          ...
          <img
            src={heroImg}
            alt=""
            className="h-full w-full object-cover opacity-45 animate-kenburns"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-ink/90 via-ink/65 to-ink/25" />
        </div>
        <div className="wrap relative flex min-h-[72vh] flex-col justify-center py-20">
          <FadeUp>
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-brass">
              <Factory size={14} /> Factory-direct · Kirti Nagar, Delhi
            </p>
            <h1 className="mt-5 max-w-2xl font-display text-4xl font-semibold leading-[1.05] text-ivory sm:text-6xl">
          ...
          <FadeUp delay={0.15}>
            <div className="mt-9 flex flex-wrap gap-3.5">
              <Link href="/collections" className="btn-brass">
                Browse the catalogue <ArrowRight size={16} />
              </Link>
              <Link
                href="/quote"
                className="btn border border-ivory/40 bg-transparent text-ivory hover:bg-ivory hover:text-ink"
              >
            ...
            <div className="mt-14 grid max-w-xl grid-cols-2 gap-6 border-t border-ivory/15 pt-8 sm:grid-cols-4">
              {stats.map((s) => (
                <div key={s.label}>
                  ...
                  <p className="mt-1 text-xs text-ivory/60">{s.label}</p>
                </div>
              ))}
            </div>
          </FadeUp>
      ...
      <div className="overflow-hidden border-y border-line bg-ivory-dark py-3.5">
        <div className="flex w-max animate-marquee gap-12 whitespace-nowrap">
          {[...Array(2)].map((_, i) => (
            <div
              key={i}
              className="flex gap-12 text-xs font-semibold uppercase tracking-[0.18em] text-walnut"
            >
          ...
          <FadeUp>
            ...
            <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3 text-sm text-ink-soft">
              <span className="flex items-center gap-2">
                <Ruler size={15} className="text-brass" /> Custom dimensions
              </span>
              <span className="flex items-center gap-2">
                <Award size={15} className="text-brass" /> Warranty included
              </span>
              <span className="flex items-center gap-2">
                <ShieldCheck size={15} className="text-brass" /> Secure delivery
              </span>
            </div>
          </FadeUp>
            ...
            <div className="grid items-center gap-10 lg:grid-cols-2">
              ...
              <div className="grid gap-4">
                {[
                  ...
                  <div
                    key={b.title}
                    className="flex gap-4 rounded-2xl border border-line bg-ivory p-5"
                  >
