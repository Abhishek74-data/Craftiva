...
title Craftiva - Optimized Master Push
cd /d "%~dp0"

echo ==========================================================
echo    CRAFTIVA FURNITURE - OPTIMIZED GITHUB PUSH
echo ==========================================================
echo.
...
echo [1/4] Optimizing images: Syncing all required catalog product photos...
if defined NODE_CMD node scripts/sync-exact-referenced-images.mjs
...
echo.
echo [2/4] Resetting heavy 4.2GB commit so GitHub accepts the push...
"!GIT_EXE!" config http.postBuffer 524288000
"!GIT_EXE!" reset origin/main

echo.
echo [3/4] Adding optimized files to Git...
"!GIT_EXE!" add -A

echo.
echo [4/4] Committing and pushing exact local site to GitHub...
"!GIT_EXE!" commit -m "Deploy 100% exact local site with all real product photos"
...
if %errorlevel% equ 0 (
    ...
    echo   local site GitHub aur Vercel par deploy ho chuki hai!
    echo ==========================================================
) else (
    echo.
